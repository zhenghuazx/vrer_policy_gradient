...
def register_models(agents):
    ...
    configuration folders to be added to rlalgorithms_tf2.agents.
    Args:
        agents: rlalgorithms_tf2.agents
...
def create_model(
    ...
    Args:
        env: gym environment.
        agent_id: str, one of the keys in rlalgorithms_tf2.agents
    ...
    try:
        model_cfg = (
            model_cfg or vrer_policy_gradient.agents[agent_id][model_type][network_type][0]
        )
    except IndexError:
        model_cfg = None
    models_folder = (
        Path(vrer_policy_gradient.agents[agent_id]['module'].__file__).parent / 'models'
    )
    assert model_cfg, (
...
def create_models(options, env, agent_id, **kwargs):
    ...
    Args:
        ...
        env: gym environment
        agent_id: str, one of the keys in rlalgorithms_tf2.agents
        **kwargs: kwargs passed to rlalgorithms_tf2.utils.common.create_model()
...
def create_agent(agent_id, agent_kwargs, non_agent_kwargs, trial=None):
    ...
    if (
        issubclass(
            vrer_policy_gradient.agents[agent_id]['agent'], vrer_policy_gradient.OffPolicy
        )
        or agent_id == 'acer'
    ):
    ...
    agent = vrer_policy_gradient.agents[agent_id]['agent'](**agent_kwargs)
