...
class Agent:
    ...
    def choose_action(self, observation):
        ...
        action = 1 if action.numpy()[0] >= 3 else action.numpy()[0]
        return action
            ...
            for i in reuse:
                for j in range(n_k):
                    ...
                    for idx, (g, state) in enumerate(zip(self.G_memory_full[i][j][:], self.state_memory_full[i][j])):
                        ...
                        action_probs = tfp.distributions.Categorical(probs=probs)
                        log_prob = action_probs.log_prob(self.action_memory_full[i][j][idx])
                        loss += - np.clip(numerator / denominator, 0.5, 2) * g * tf.squeeze(log_prob)
...
if __name__ == '__main__':
    ...
    path = "/content/drive/MyDrive/vrer/VPGVRER/{}/approx-2nd-seed-{}-n_k-{}-id-{}-c-{}".format(problem, seed, n_k,
                                                                                                index, c)
    # Log details
    ...
    agent = Agent(alpha=0.0003, gamma=0.99, n_actions=num_actions.n, n_k=n_k, num_episodes=num_episodes, c=c)
    score_history = []
