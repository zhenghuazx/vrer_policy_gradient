...
class Executor:
    ...
    def maybe_create_agent(self, argv):
        ...
        assert command in vrer_policy_gradient.commands, f'Invalid command `{command}`'
        to_display.update(vrer_policy_gradient.commands[command][0])
        if total == 1:
            self.display_commands({command: to_display})
            return
        agent_id = argv[1]
        assert agent_id in vrer_policy_gradient.agents, f'Invalid agent `{agent_id}`'
        to_display.update(vrer_policy_gradient.agents[agent_id]['module'].cli_args)
        if total == 2:
            title = f'{command} {agent_id}'
            if (
                issubclass(vrer_policy_gradient.agents[agent_id]['agent'], OffPolicy)
                or agent_id == 'acer'
            ):
    ...
    def parse_known_args(self, argv, tuning=False):
        ...
        self.add_args(
            vrer_policy_gradient.agents[self.agent_id]['module'].cli_args,
            agent_parser,
            tuning,
        )
        self.add_args(
    ...
    def execute(self, argv):
        ...
        if self.command == 'tune':
            ...
            getattr(self.agent, vrer_policy_gradient.commands[self.command][1])(
                **vars(command_known)
            )
