...
import os
from vrer_policy_gradient import TRPOVRER
...
for i in [0, 1, 4]:
    ...
    actor_model_cfg = 'TRPO/models/ann-actor.cfg'  # vrer_policy_gradient.agents['trpo']['actor_model']['ann'][0]
    critic_model_cfg = 'TRPO/models/ann-critic.cfg'  # vrer_policy_gradient.agents['trpo']['critic_model']['ann'][0]
    ...
    agent = TRPOVRER(envs, actor_model, critic_model, seed=seed, n_steps=n_steps, entropy_coef=entropy_coef, mini_batches=mini_batches,
                 clip_norm=clip_norm, checkpoints=checkpoints, history_checkpoint=history_checkpoint, log_frequency=4, buffer_size=buffer_size, num_reuse_each_iter=2)
